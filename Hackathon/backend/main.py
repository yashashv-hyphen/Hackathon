...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://127.0.0.1:3000", "http://localhost:5500", "http://127.0.0.1:5500", "http://localhost:8080", "http://127.0.0.1:8080", "null"],
...
@app.post("/",response_model=ExperimentResponse)
@app.post("/process",response_model=ExperimentResponse)#homepage
def upload_lab_manual(file: UploadFile = File(...),db:Session=Depends(get_db)):
    ...
    try:
        ...
        db.add(database_models.Precaution(
            ...
            precaution=modified_json["precautions"]
        ))
        db.commit()
...
def feedback(user_input: ActionRequest,db:Session=Depends(get_db)):
    ...
    precautions = precaution_row.precaution or []
...
def chatbot(user_input: ChatbotRequest, db: Session = Depends(get_db)):
    ...
    precautions_list = precaution_row.precaution if precaution_row else []
